...
public class GameServiceStub implements GameService {
    ...
    private Comparator<Game> comparatorFor(String sortField) {
        return switch (sortField) {
            ...
            case "status" -> Comparator.comparing((Game g) -> g.status).thenComparing(g -> g.tipoff);
            case "tipoff" -> Comparator.comparing((Game g) -> g.tipoff).thenComparing(g -> g.id);
            default       -> Comparator.comparing((Game g) -> g.tipoff).thenComparing(g -> g.id);
        };
    }
    ...
    private static long stableIdFromName(String name) {
